...
export const ObserveInteraction = ({player, inactivityPeriod=3000, onSleep, onWake}) => {
  let autohideTimeout;
  const Wake = event => {
    if(event.type === "keydown" &&  player.playerOptions.keyboardBackKeys.includes(event.code)) {
...
const SpatialNavKeydownHandler = ({player, event, setRecentUserAction, SeekHandler, onHideUi}) => {
  ...
  if(player.playerOptions.keyboardBackKeys.includes(event.key) && player.controls.IsVisible()) {
    onHideUi?.();
    return true;
  }
  switch (event.key) {
