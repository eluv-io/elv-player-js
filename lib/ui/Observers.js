...
const SpatialNavKeydownHandler = ({player, event, setRecentUserAction, SeekHandler, onHideUi}) => {
  ...
  const liveIndicatorFocused = player.controls.GetPlayerControl(ElvPlayerControlIds.live_toggle)?.contains(currentFocus);
  const behindLiveEdge = player.behindLiveEdge;

  switch (event.key) {
    case "ArrowUp":
      if(seekbarHasFocus && behindLiveEdge) {
    ...
    case "Enter":
      if(liveIndicatorFocused) {
        player.controls.GetPlayerControl(ElvPlayerControlIds.live_toggle)?.click();
        return true;
      }
