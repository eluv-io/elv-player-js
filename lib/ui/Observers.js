...
export const RegisterModal = ({player, element, Close, IsSubmenuOpen, CloseSubmenu}) => {
  ...
  const onKeyListener = event => {
    ...
    } else if(player.playerOptions.keyboardControls === EluvioPlayerParameters.keyboardControls.SPATIAL_NAVIGATION) {
...
export const ObserveInteraction = ({player, inactivityPeriod=3000, onSleep, onWake}) => {
  let autohideTimeout;
  const Wake = event => {
    if(event.type === "keydown" &&  player.playerOptions.keyboardBackKeys.includes(event.key)) {
...
const ShouldHandleKeyboardEvent = ({player, event}) => {
  ...
  if(player.playerOptions.keyboardControls === EluvioPlayerParameters.keyboardControls.SPATIAL_NAVIGATION) {
    return true;
  }
