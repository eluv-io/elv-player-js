...
export const RegisterModal = ({player, element, Close, IsSubmenuOpen, CloseSubmenu}) => {
  if(!element) { return; }

  const onKeyListener = event => {
    if(player.playerOptions.keyboardControls === EluvioPlayerParameters.keyboardControls.ON) {
      if(event.key === "Escape") {
        Close();
        event.preventDefault();
      }
    } else if(player.controls.IsSpatialNavigationEnabled()) {
      if(player.playerOptions.keyboardBackKeys.includes(event.key) || event.key === "ArrowLeft") {
        // Close modal/submenus on Esc/Backspace/LeftArrow press
        if(IsSubmenuOpen?.()) {
          CloseSubmenu?.();
        } else {
          Close();
      ...
      switch (event.key) {
        case "ArrowRight":
          // Simulate click on right arrow press
          event.target?.click();
          event.preventDefault();
          break;
        case "ArrowUp":
  ...
  let registerTimeout = setTimeout(() => {
    document.body.addEventListener("elv-player-close-modal", Close);
    element.addEventListener("keydown", onKeyListener);
  ...
  return () => {
    ...
    document.body.removeEventListener("elv-player-close-modal", Close);
    element.removeEventListener("keydown", onKeyListener);
...
export const ObserveInteraction = ({player, inactivityPeriod=3000, onSleep, onWake}) => {
  let autohideTimeout;
  const Wake = event => {
    if(event.type === "keydown" && player.controls.IsSpatialNavigationEnabled()) {
      if(player.playerOptions.keyboardBackKeys.includes(event.key)) {
        // Since "Back" keys hide the UI, we don't want to wake it up when they are pressed.
        return;
      } else if(event.key !== "Enter" && !player.controls.IsVisible()) {
        // This event is going to wake up the UI, prevent all keys except Enter (play/pause) from taking effect.
        event.preventDefault();
      }
    }
 ...
 * @param player {EluvioPlayer}
 */
const GetControlContainer = (player) => {
  return player.controls.GetPlayerControl(ElvPlayerControlIds.bottom_controls_container) ||
    player.controls.GetPlayerControl(ElvPlayerControlIds.info_box);
};
 ...
 * @param player {EluvioPlayer}
 */
const GetFocusableElements = (player) => {
  return Array.from(
    GetControlContainer(player)
      ?.querySelectorAll("button, [href], input, select, textarea, [tabindex]:not([tabindex=\"-1\"])")
  ).filter((el) => !el.hasAttribute("disabled") && el.offsetParent !== null);
};

/**
 * Move focus to the next or previous focusable element in the control container.
 * Should simulate a Tab press pretty accurately.
 * @param player {EluvioPlayer}
 * @param direction {"next" | "prev"}
 */
const MoveFocus = ({player, direction}) => {
  const delta = direction === "prev" ? -1 : 1;
  const focusableElements = GetFocusableElements(player);
  const activeIndex = focusableElements.indexOf(document.activeElement);
  const nextIndex = activeIndex + delta;
  if(nextIndex >= 0 || nextIndex < focusableElements.length) {
    // Only move focus if the current active element is not the first or last focusable element (don't wrap focus).
    focusableElements[nextIndex]?.focus();
  }
};

const SpatialNavKeydownHandler = ({player, event, setRecentUserAction, SeekHandler, onHideUi}) => {
  const playerCanHandleBack = player.controls.IsVisible() && !player.loading && !player.__error;
  if (player.playerOptions.keyboardBackKeys.includes(event.key) && playerCanHandleBack) {
    onHideUi?.();
    return true;
  }

  const currentFocus = document.activeElement;
  const controlsRowHasFocus = GetControlContainer(player)?.contains(currentFocus);
  const seekbarHasFocus = player.controls.GetPlayerControl(ElvPlayerControlIds.seekbar)?.contains(currentFocus);

  const dvrEnabled = player.controls.IsDVREnabled();
  const dvrAvailable = player.controls.IsDVRAvailable();
  const dvrTogglesHaveFocus = dvrAvailable &&
    (player.controls.GetPlayerControl(ElvPlayerControlIds.dvr_toggle)?.contains(currentFocus) ||
      player.controls.GetPlayerControl(ElvPlayerControlIds.live_toggle)?.contains(currentFocus));

  switch (event.key) {
    case "ArrowUp":
      if(dvrAvailable && (seekbarHasFocus || !dvrEnabled)) {
        if(dvrEnabled) {
          player.controls.GetPlayerControl(ElvPlayerControlIds.dvr_toggle)?.focus();
        } else {
          player.controls.GetPlayerControl(ElvPlayerControlIds.live_toggle)?.focus();
        }
        return true;
      }
      if(controlsRowHasFocus) {
        player.controls.GetPlayerControl(ElvPlayerControlIds.seekbar)?.focus();
        return true;
      }
      return false;
    case "ArrowDown":
      if(seekbarHasFocus || (dvrTogglesHaveFocus && !dvrEnabled)) {
        player.controls.GetPlayerControl(ElvPlayerControlIds.play_pause)?.focus();
        return true;
      }
      if(dvrTogglesHaveFocus && dvrEnabled) {
        player.controls.GetPlayerControl(ElvPlayerControlIds.seekbar)?.focus();
        return true;
      }
      return false;
    case "ArrowLeft":
      if(dvrTogglesHaveFocus) {
        player.controls.SetDVREnabled(false);
        player.controls.GetPlayerControl(ElvPlayerControlIds.live_toggle)?.focus();
        return true;
      }
      if(controlsRowHasFocus) {
        MoveFocus({player, direction: "prev"});
        return true;
      }
      return false;
    case "ArrowRight":
      if(dvrTogglesHaveFocus) {
        player.controls.SetDVREnabled(true);
        player.controls.GetPlayerControl(ElvPlayerControlIds.dvr_toggle)?.focus();
        return true;
      }
      if(controlsRowHasFocus) {
        MoveFocus({player, direction: "next"});
        return true;
      }
      return false;
    case "Enter":
      if(dvrTogglesHaveFocus) {
        const newDvrState = !dvrEnabled;
        player.controls.SetDVREnabled(newDvrState);
        if(newDvrState) {
          player.controls.GetPlayerControl(ElvPlayerControlIds.dvr_toggle)?.focus();
        } else {
          player.controls.GetPlayerControl(ElvPlayerControlIds.live_toggle)?.focus();
      ...
      // Don't return - let other handlers process Enter.
      break;
  }
  return DefaultKeydownHandler({player, event, setRecentUserAction, SeekHandler});
};

const ShouldHandleKeyboardEvent = ({player, event}) => {
  if(event.defaultPrevented) {
    return false;
  }

  // In spatial-nav mode, we handle keyboard events regardless of focus state.
  if(player.controls.IsSpatialNavigationEnabled()) {
    return true;
  }

  // Don't handle keyboard events if player profile form is visible
  if(player.__showPlayerProfileForm) {
    return false;
  }

  // Keyboard controls should only fire if the player is in focus
  return player.target.contains(event.target);
};

export const ObserveKeydown = ({player, setRecentUserAction, onHideUi}) => {
  ...
  const onKeydown = event => {
    if(!ShouldHandleKeyboardEvent({player, event})) {
      return false;
    }

    let result;
    if(player.controls.IsSpatialNavigationEnabled()) {
      result = SpatialNavKeydownHandler({player, event, setRecentUserAction, SeekHandler, onHideUi});
    } else {
