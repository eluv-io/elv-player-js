...
export const ObserveResize = ({target, setSize, setOrientation, setDimensions}) => {
  ...
  const ResizeUpdate = () => {
    ...
    if(dimensions.width > 1400 && dimensions.height > 600) {
      ...
      size = "md";
    } else if(dimensions.width > 450 && dimensions.height > 200) {
      size = "sm";
    }
...
const SpatialNavKeydownHandler = ({player, event, setRecentUserAction, SeekHandler, onHideUi}) => {
  ...
  const seekbarRef = player.controls.GetPlayerControl(ElvPlayerControlIds.seekbar);
  const seekbarHasFocus = seekbarRef?.contains(currentFocus);
  const playButtonRef = player.controls.GetPlayerControl(ElvPlayerControlIds.play_pause);
  const playButtonHasFocus = playButtonRef?.contains(currentFocus);

  const liveToggleRef = player.controls.GetPlayerControl(ElvPlayerControlIds.live_toggle);
  const liveIndicatorFocused = liveToggleRef?.contains(currentFocus);
  ...
  switch (event.key) {
    case "ArrowUp":
      ...
      return false;
    case "ArrowDown":
      if (seekbarHasFocus || playButtonHasFocus) {
