...
export class EluvioPlayer {
  ...
  async __InitializeHLS({playoutUrl, authorizationToken, drm, drms, multiviewOptions}) {
    ...
    const nativeHLSSupported = this.video.canPlayType("application/vnd.apple.mpegURL") || this.video.canPlayType("audio/mpegurl");
    if((nativeHLSSupported && isMobileOrTablet) || [EluvioPlayerParameters.drms.FAIRPLAY, EluvioPlayerParameters.drms.SAMPLE_AES].includes(drm) || !this.HLS.isSupported()) {
  ...
  __SetCasting(casting) {
    ...
    if(this.casting) {
      const castControls = this.castHandler.GetControls();
      const overrideControls = new PlayerControls({player: this});
      Object.keys(castControls).forEach(methodName =>
        overrideControls[methodName] = castControls[methodName]
      );

      this.__defaultControls = this.controls;
      this.__defaultControls.Pause();
      this.controls = overrideControls;
    } else {
      this.controls = this.__defaultControls;
      delete this.__defaultControls;
    }
