...
export class EluvioPlayer {
  ...
  __RegisterVideoEventListener(event, callback) {
    ...
    const Disposer = () => this.video.removeEventListener(event, callback);
    this.__listenerDisposers.push(Disposer);

    return () => {
      Disposer();
      this.__listenerDisposers = this.__listenerDisposers.filter(d => d !== Disposer);
    };
  }
  ...
  async __PlayoutOptions() {
    ...
    const { playoutUrl, drms, thumbnailTrack, properties } = this.sourceOptions.playoutOptions[protocol].playoutMethods[drm];
    ...
    return {
      ...
      offeringURI,
      offering: offeringId,
      compositionKey: playoutParameters.channel,
  ...
  async __Initialize(parameters, restartParameters) {
    ...
    try {
      ...
      const playbackStartedDisposer = this.__RegisterVideoEventListener("timeupdate", () => {
        this.reloads = 0;
        this.playbackStarted = true;
        this.__SettingsUpdate();
        playbackStartedDisposer();
      });
      ...
      if(this.playerOptions.loadChapters) {
        this.__LoadChapters({...(this.playoutInfo || {})})
          .then(chapterTags => this.__chapterTags = chapterTags)
  ...
  async __InitializeDash({playoutUrl, authorizationToken, drm, drms}) {
    this.Dash = (await import("dashjs")).MediaPlayer;
    const dashPlayer = this.Dash().create();
  ...
  __Reset() {
    ...
    this.casting = false;
    this.__error = undefined;
    this.__chapterTags = undefined;
    this.playoutInfo = undefined;
  }
  ...
  async __LoadChapters({objectId, offering, compositionKey, clipStart, clipEnd}) {
    ...
    return ((await this.__QueryAIAPI({
      objectId,
      ...
      .filter(tag => typeof tag.start_time !== "undefined" && typeof tag.tag === "string")
  ...
  async __QueryAIAPI({
    ...
    const authToken =
      (this.sourceOptions.playoutParameters || {}).authorizationToken ||
      new URL(await client.FabricUrl({
    ...
    return !format ? response :
      await client.utils.ResponseToFormat(format, response);
  }
}
