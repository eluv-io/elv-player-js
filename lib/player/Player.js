...
export class EluvioPlayer {
  ...
  async __RedeemCode(code) {
    ...
    await (await this.__Client()).RedeemCode({
  ...
  async __Initialize(parameters, restartParameters) {
    ...
    try {
      ...
      if(this.collectionInfo && this.collectionInfo.isPlaylist && this.collectionInfo.mediaIndex < this.collectionInfo.mediaLength - 1) {
        this.__RegisterVideoEventListener("ended", () => this.controls && this.controls.CollectionPlayNext());
      }
  ...
  async __InitializeDash({playoutUrl, authorizationToken, drm, drms, multiviewOptions}) {
