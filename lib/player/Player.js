...
import PlayerControls, {CreateElement, InitializeTicketPrompt, PlayPause} from "../PlayerControls.js";
...
export class EluvioPlayer {
  constructor(target, video, parameters) {
    try {
      if(
    ...
    this.Initialize(video, parameters);
  ...
  async Initialize(video, parameters, restartParameters) {
    if(this.__destroyed) { return; }
    ...
    if(["fairplay", "sample-aes"].includes(drm) || !this.HLS.isSupported()) {
      ...
      hlsPlayer.on(this.HLS.Events.ERROR, async (event, error) => {
        ...
        if(error.response && error.response.code === 403) {
          ...
          if(error.fatal) {
            if(error.data.type === this.HLS.ErrorTypes.MEDIA_ERROR) {
