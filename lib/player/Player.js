...
export class EluvioPlayer {
  ...
  async __PlayoutOptions() {
    ...
    if(this.collectionInfo) {
      ...
      this.sourceOptions.playoutParameters.objectId = client.utils.DecodeVersionHash(activeMedia.mediaHash).objectId;
    ...
    try {
      ...
      if(restartParameters) {
        ...
        if(restartParameters.currentTime) {
          this.RegisterVideoEventListener(
            "loadedmetadata",
      ...
      this.RegisterVideoEventListener("canplay", () => {
      ...
      this.RegisterVideoEventListener("play", () => {
        this.playbackStarted = true;
        this.__SettingsUpdate();
      });

      // Detect live video
      this.RegisterVideoEventListener("durationchange", () => {
      ...
      if(this.collectionInfo && this.collectionInfo.isPlaylist && this.collectionInfo.mediaIndex < this.collectionInfo.mediaLength - 1) {
        this.RegisterVideoEventListener("ended", () => this.controls && this.controls.CollectionPlayNext());
      }
