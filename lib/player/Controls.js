...
class PlayerControls {
  ...
  /**
   * Get a reference to a player control element, if it exists.
   * @param id {ElvPlayerControlIds} - The generic ID of the control to retrieve
   */
  GetPlayerControl(id){
    return document.getElementById(this.__GetPlayerControlId(id));
  }

  /**
   * Converts generic IDs to IDs that are unique to this player instance.
   * @param id {ElvPlayerControlIds} - The generic ID of the control to retrieve
   * @return {string} - The unique ID of the control, as it should appear in the DOM.
   */
  __GetPlayerControlId(id){
    return `${id}-${this.player.playerId}`;
  }
  ...
  /**
   * Closes any open menus or info boxes but doesn't hide the controls.
   *
   * @methodGroup Controls
   */
  CloseMenu() {
    document.body.dispatchEvent(new Event("elv-player-close-modal"));
  }
  ...
  GetQualityLevels() {
    ...
    if(this.player.hlsPlayer) {
      ...
      levels = this.player.dashPlayer.getRepresentationsByType("video")
        .map((level) => ({
          index: level.index,
          active: level.index === this.player.dashPlayer.getCurrentRepresentationForType("video").index,
          resolution: `${level.width}x${level.height}`,
          bitrate: level.bitrateInKbit * 1000,
          label: `${level.width}x${level.height} (${(level.bitrateInKbit / 1000).toFixed(1)}Mbps)`,
  ...
  SetQualityLevel(levelIndex) {
    if(this.player.hlsPlayer) {
      ...
      this.player.dashPlayer.setRepresentationForTypeByIndex("video", levelIndex);
  ...
  IsSpatialNavigationEnabled() {
    return this.player.playerOptions.ui === EluvioPlayerParameters.ui.TV &&
      this.player.playerOptions.keyboardControls === EluvioPlayerParameters.keyboardControls.SPATIAL_NAVIGATION;
  }
  ...
  /**
   * Retrieve whether thumbnails are loaded and available for this content.
   *
   * @methodGroup Thumbnails
   *
   * @returns {boolean} - Whether or not thumbnails are available
   */
  ThumbnailsAvailable() {
    return this.player.thumbnailHandler && this.player.thumbnailHandler.loaded || false;
  }

  /**
   * Retrieve a thumbnail for the specified time, if available
   *
   * @methodGroup Thumbnails
   * @param {number} time - The time to retrieve a thumbnail for
   *
   * @returns {string || undefined} - A data URL for the thumbnail image
   */
  GetThumbnailImage(time) {
    if(!this.ThumbnailsAvailable()) {
      return;
    }

    return this.player.thumbnailHandler.ThumbnailImage(time);
  }

  GetVideoDimensions() {
    return this.player.video.getBoundingClientRect();
  }
