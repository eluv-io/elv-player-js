...
export const InitializeMuxMonitoring = async ({
  ...
  const client = await elvPlayer.__Client();
  ...
  if(elvPlayer.player) {
    if(elvPlayer.HLS) {
      ...
      options.dashjs = elvPlayer.dashPlayer;
    }
  }
  try {
    ...
    elvPlayer.Log("elv-player-js: Mux monitoring initialized");
    // eslint-disable-next-line no-console
    elvPlayer.Log(JSON.stringify({...options, hlsjs: {}}));
  } catch (error) {
    // eslint-disable-next-line no-console
    elvPlayer.Log("elv-player-js: Failed to initialize mux monitoring:", true);
    // eslint-disable-next-line no-console
    elvPlayer.Log(JSON.stringify(options, null, 2), true);
    // eslint-disable-next-line no-console
    elvPlayer.Log(error, true);
  }
};
