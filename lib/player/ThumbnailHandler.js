...
const FormatVTTCue = ({label, cue, offset=0}) => {
...
export const ParseVTTTrack = async (track, offset=0) => {
  ...
  Array.from(textTrack.cues)
    .forEach(cue => {
      const parsedCue = FormatVTTCue({label: track.label, cue, offset});
...
class ThumbnailHandler {
  constructor(thumbnailTrackUrl, offset) {
  ...
  async LoadThumbnails() {
    ...
    let tags = await ParseVTTTrack({label: "Thumbnails", vttData, offset: this.offset});
