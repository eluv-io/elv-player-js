...
class Cast {
  ...
  async SetMedia({playoutOptions, playoutParameters}) {
    // Get dash options
    let options = playoutOptions?.dash?.playoutMethods?.clear || playoutOptions?.dash?.playoutMethods?.widevine;
    ...
    if(!options) {
      ...
      try {
        ...
        options = playoutOptions?.dash?.playoutMethods?.clear || playoutOptions?.dash?.playoutMethods?.widevine;
      } catch(error) {
  ...
  async Start() {
    ...
    try {
      ...
      const { title, subtitle, image } = (this.player.controls.GetContentInfo() || {});
  ...
  Disconnect(force) {
    ...
    this.connected = false;
    this.player?.__SetCasting(false);
  }
